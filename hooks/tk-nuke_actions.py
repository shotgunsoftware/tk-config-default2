...
class NukeActions(HookBaseClass):
    ...
    def _create_read_node(self, path, sg_data):
        ...
        seq_range = self._find_sequence_range(path, sg_data)

        # to fetch the nuke prefs from pipeline
        nuke_prefs = NukePreferences()

        for knob_name, knob_value in nuke_prefs.getKnobOverridesGenerator(self._find_pipe_step(path, sg_data)):
    ...
    def _find_pipe_step(self, path, sg_data):
        ...
        Args:
            path (str): File path
            sg_data (dict): Shotgun data dictionary with all the standard publish fields.
        ...
        if sg_data["entity"].get("type") == "Element":
            ...
            if sg_data["task"]:
                entity_type = sg_data["task"].get("type")
                filters = [["id", "is", sg_data["task"].get("id")]]
                fields = ["step"]
    ...
    def _find_sequence_range(self, path, sg_data):
        """
        ...
        if sg_data["entity"].get("type") == "Element":
            ...
            if sg_data["version"]:
                entity_type = sg_data["version"].get("type")
                filters = [["id", "is", sg_data["version"].get("id")]]

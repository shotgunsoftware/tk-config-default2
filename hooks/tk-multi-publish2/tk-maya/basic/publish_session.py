...
class MayaSessionPublishPlugin(HookBaseClass):
    ...
    def publish(self, settings, item):
        ...
        # The actual file to publish will be published from the work path
        # Backup the session path to restore it at finalize
        item.properties["local_path"] = path
        ...
        try:
            ...
            self.logger.error("Could not copy Maya scene file to {}: exception {}.".format(work_path))
            return False

        # update the item with the work path
        item.properties["path"] = work_path

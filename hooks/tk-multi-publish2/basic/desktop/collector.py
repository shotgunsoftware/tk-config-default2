...
class BasicSceneCollector(HookBaseClass):
    ...
    def process_file(self, settings, parent_item, path):
        ...
        for info in path_info['path_info_returns']:
            ...
            # self.logger.warning(">>>>> camera? %s" % properties.get('camera'))
    ...
    def _json_properties(self, item):
        ...
        for i in process_jobs:
            ...
            current_process['nuke_settings']['main_read'].update( {
                                        ...
                                        'first': int(item.properties['frame_range'].split('-')[0]),
                                        'last': int(item.properties['frame_range'].split('-')[-1]),
                                        } )
            # Shot CCC
            shot_ccc_file = ""
            ...
            output_format_name = ""
            if output_format:
                output_format_name = output_format.get('code')
            ...
            # self.logger.warning(">>>>> main_reformat: %s" % current_process['nuke_settings']['main_reformat'] )

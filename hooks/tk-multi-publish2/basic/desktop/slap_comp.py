...
class SlapCompPlugin(HookBaseClass):
    """
    ...
    def accept(self, settings, item):
        ...
        if extension in valid_extensions:
            ...
            if item.properties.sg_slap_comp:
    ...
    def validate(self, settings, item):
        ...
        if not item.properties.get("step"):
            review_process = publisher.shotgun.find("Step", 
        ...
        if not publish_name:
            ...
            version_number = str(version_number).zfill(3)
            version_name = publish_name + "_v" + version_number
        ...
        if existing_version:
            self.logger.warn(
                "Please update to the next Version or send the conflicting Version to thrash",
                extra={
            ...
            self.logger.info("Creating Version on SG called: %s" % (version_name))
        ...
        if item.context.entity['type'] == "Shot":
            ...
            # Test for Published Main Plate - required for certain processes
            main_plate = self._get_published_main_plate(sg_reader, item) 
            if (not main_plate and
            item.properties.sg_slap_comp):
                raise Exception("Could not find Published Main Plate for %s. This is required for slap comp." %(item.context.entity['name']))
            else:
                shot_info.update({"main_plate":main_plate})
                self.logger.info("Got main plate of entity %s - %s" %(str(item.context.entity['id']),main_plate))
        ...
        if (len(item.properties.get("project_info")['sg_review_qt_codecs'])>0 and codecs): 
            ...
            if review_codecs:
                for i in review_codecs:
                    ...
                    self.logger.info("Nuke codec name: %s" % (review_codec,))
    ...
    def publish(self, settings, item):
        ...
        for i in process_dict.keys():
            ...
            if resolve_fields['version'] == None:
                self.logger.warn("No version number find from path. Setting to version Zero")
    ...
    def _get_published_main_plate(self, sg_reader, item):
        ...
        entity_filters.append( [ "project.Project.id", "is", item.context.project['id'] ] )
        entity_filters.append( {
                                "filter_operator": "any",
                                "filters": [
                                    ["entity.Shot.id", "is", item.context.entity.get("id")],
                                    ["entity.Asset.id", "is", item.context.entity.get("id")],
                                ]
                            } )
    ...
    def set_process_info(self, dl_module, plugin_name, process_name, plugin_settings, project_info, software_info, entity_info, item, multiple_task_list=None):
        ...
        try:
            ...
            self.logger.warn("Could not get user_name info")
        ...
        process_info = dict(
            ...
            plate_type = "", # Should pull from linked main plate data if available
            ...
            sg_slap_comp = item.properties.sg_slap_comp
            )

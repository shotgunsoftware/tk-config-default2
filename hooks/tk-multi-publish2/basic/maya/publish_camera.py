...
class MayaCameraPublishPlugin(HookBaseClass):
    ...
    def accept(self, settings, item):
        ...
        * ``checked``: If ``True``, the created task will be checked in the UI,
        ...
        self.logger.debug("Camera name %s"%str(cam_name))
    ...
    def validate(self, settings, item):
        ...
        work_fields["maya.camera_name"] = cam_name
    ...
    def publish(self, settings, item):
        ...
        # the camera to publish need to publish the Transform node otherwise
        #The Alembic export fails.
        cam_shape = item.properties["camera_shape"]

        # make sure it is selected
        #cmds.ls(cam_shape)
        self.logger.debug("publish_camera : select cam %s"%str(cam_shape))
        camShape=cmds.ls(cam_shape)
        cam_trans=cmds.listRelatives(camShape, parent=True)[0]
        self.logger.debug("transform %s"%cam_trans)
        ...
        alembic_args.append("-root %s " % cam_trans)
        ...
        # Abc Export Command 
        # AbcExport -j "-worldSpace -stripNamespaces -root camMainTT50mm -file test.abc"
    ...
    def _cam_name_matches_settings(self, cams, settings):
        """
        ...
        self.logger.debug("Cam_patterns: %s"%str(cam_patterns))

        # if no patterns specified, then no constraints on camera name
        if not cam_patterns:
            self.logger.debug("No camera patterns defined")
            return True

        for camera_pattern in cam_patterns:
            #self.logger.debug("cam_pattern %s match %s"%(camera_pattern,cams))
            if fnmatch.fnmatch(cams, camera_pattern):
                return True
    ...
    def _find_scene_animation_range(self):
        ''' 
            find the scene animation range
        '''
        anim_curves = cmds.ls(typ="animCurve")
        if not anim_curves:
            return (1, 1)

        self.logger.debug("find_scene_animation_range")

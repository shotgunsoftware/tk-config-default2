...
class MayaSessionPublishPlugin(HookBaseClass):
    ...
    def validate(self, settings, item):
        ...
        # check for existing master scene JSON
        if item.properties.get('is_masterscene'):
            if item.properties.get('json_exists'):
                self.logger.debug(">>>> JSON file already exists for this master scene")
            else:
                pass
                # with open(item.properties.get('json_path'), "w+") as file:

...
class BasicFilePublishPlugin(HookBaseClass):
    ...
    def accept(self, settings, item):
        ...
        if item.properties.get("publish_to_shotgun"):
    ...
    def publish(self, settings, item):
        ...
        if publish_thumbnail:
            ...
            self.logger.warn("No thumbnail file given.")
        ...
        if "sg_publish_path" in item.parent.properties:
            publish_dependencies_ids.append(item.parent.properties.get("sg_publish_path"))

        #### DISABLED COPYING FUNCTIONALITY ####
        ...
        if version:
            publish_status = "lwv"
            self.logger.info("Found associated Version: %s" % (version['code']))
        else:
            publish_status = "cmpt"
    ...
    def get_publish_template(self, settings, item):
        ...
        return item.properties.get("publish_template")
            ...
            for type_def in settings["File Types"].value:
                ...
                if extension in file_extensions:
                    ...
                    self.review_process['sg_department'] == "3D"):
                        ...
                        publish_thumb = publish_thumb_jpeg.apply_fields(item.properties.get("file_fields"))
                        self.logger.debug("Converted thumb output: %s" % (publish_thumb))
                        item.properties.publish_thumb = publish_thumb
    ...
    def get_publish_name(self, settings, item):
        ...
        if publish_name:
            self.logger.info("Got publish name from item!")
            return publish_name

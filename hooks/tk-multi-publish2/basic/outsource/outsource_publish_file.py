...
class BasicFilePublishPlugin(HookBaseClass):
    ...
    def accept(self, settings, item):
        ...
        self.logger.warning(">>>>> process_plugin_info: %s" % item.get_property('process_plugin_info'))
        outsource = False
        if item.get_property('process_plugin_info'):
            outsource = item.properties['process_plugin_info'].get('outsource')
    ...
    def validate(self, settings, item):
        ...
        # for i in item.properties.items():
        #     self.logger.warning(">>>>> %s: %s" % (i[0],i[-1]))
